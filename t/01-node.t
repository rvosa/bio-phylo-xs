...
my $node1 = $fac->create_node;
my $node2 = $fac->create_node;
my $node3 = $fac->create_node;
...
ok( looks_like_instance( $@, 'Bio::Phylo::Util::Exceptions::ObjectMismatch' ),
    '62 set bad taxon' );
undef($@);
ok( $node1->set_taxon( $fac->create_taxon ), '63 set good taxon' );
eval { $node1->set_taxon( $fac->create_node ) };
ok( looks_like_instance( $@, 'Bio::Phylo::Util::Exceptions::ObjectMismatch' ),
    '64 set bad taxon' );
undef($@);
